...
def test_generator_returns_exact_day1_shape():
    ...
    assert df.shape == (100, 18)
    assert list(df.columns) == [
        "attempt_id", "payment_id", "customer_id", "amount_inr",
...
def test_dataset_contract_exposes_feature_label_and_outcome_groups():
    contract = dataset_contract()
    assert contract["rows"] == 5000
    assert contract["columns"] == 18
